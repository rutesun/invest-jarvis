...
`WebFetch`는 네이버 뉴스 등 일부 도메인에서 차단된다. URL이 fetch 안 될 경우 curl로 직접 가져온다:
...
네이버 뉴스 기준으로 동작 확인됨. 제목/설명은 og 태그에서도 추출 가능.

## 작업 방식

1. 사용자가 기록할 내용을 주면 → 타입(trades/thoughts/research) 판단
2. URL만 주어진 경우 → WebFetch 시도, 실패 시 curl로 본문 추출
3. `notebook/{type}/YYYY-MM.md` 읽기 (없으면 신규 생성)
4. 해당 날짜 H2가 있으면 그 아래, 없으면 날짜 H2 추가 후 항목 append
5. 관련 항목이 다른 파일에 있으면 위키링크로 연결
6. 파일 변경 후 "어느 파일 어느 섹션에 추가했다"고 한 줄 보고
