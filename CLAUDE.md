...
기능 변경(`src/`)이 있는 PR을 마무리할 때는 `/change-record`로 `docs/changes/` 변경 기록을 작성하고 `docs/changes/INDEX.md`를 갱신한다. pre-push에서 LLM이 기능 변경으로 판정하면 `docs/FEATURES.md`와 `docs/changes/` 누락 시 push가 차단된다.
...
### 모델 선택
- 충분한 설계(스펙·구현 계획)가 이미 있는 코딩 작업은 최상위 모델 대신 적정 티어 모델(예: Claude Sonnet, GPT Terra급)에 위임한다. 특정 모델을 강제하는 것이 아니라, 작업 난이도에 맞는 모델을 고르라는 원칙이다.
- 최상위 모델은 설계·리뷰·까다로운 디버깅처럼 판단이 어려운 단계에 아껴 쓴다.
