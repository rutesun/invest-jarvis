...
- 구조 레벨
  ...
  - zone(구간) 중심으로 표시
- 실행 레벨
  - `pivot / 이동평균선 / 기타 단기 보조선` 중 가까운 순서로 표시
  - 진입/확인/리스크 관리용 line 정보
- 팩터 분류
  - `주도 / 보조 / 참고`
  - 각 팩터별 이유(`role_reason`) 표시
...
구조 레벨은 장기 구조 구간이고, 실행 레벨은 pivot / MA / 패턴 기반 단기 실행 보조선입니다.
...
> **참고**: llm 섹션은 루트 config.yaml에서만 읽는다 (`--config-path`는 normalize 전용).
