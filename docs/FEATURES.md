...
**US 데이터 신선도 (1분봉 백필):**
- 야후 일봉 피드가 최근 완성 세션을 NaN으로 비우는 경우가 있음(미국장 시간대 이슈). 이때 1분봉(정규장 09:30–16:00 ET, 최근 ~7일)을 일봉 OHLCV로 집계해 NaN 봉을 채움 (`backfill_daily_from_intraday`, `yfinance_provider`).
- 종가 근처(≥15:55)까지 도달한 **완성 세션만** 사용 — 장중 미완성 세션과 기존 정상 봉은 건드리지 않음. 7일 초과 stale는 미커버(향후 staleness 경고로 보완).
- 효과: 일봉이 stale할 때 헤더 가격·전 지표(SMA·MACD·RS 등)가 최신 완성 세션 기준으로 정정됨(예: stale 데드크로스 → fresh 골든크로스).
...
**버그 수정 (실데이터 검증):**
- **2026-06-17: 매도 규칙 재정의 + 이동평균 컬럼명 버그 (Critical)** — `exit_rules.py`가 `SMA150`/`SMA200`/`SMA50`(언더스코어 없이)을 조회했으나 실제 `raw_dataframe`은 `SMA_150` 등(언더스코어)이라 **단기·장기 이평 매도 신호와 추적 손절이 실전에서 전혀 작동하지 않았다**(테스트가 가짜 컬럼명으로 통과해 미발견). 컬럼명을 `SMA_*`로 수정해 부활. 장기 이탈 규칙은 사용자 정의대로 재정의: 평균 거래량(`Vol_SMA_20`)×1.2 이상을 동반한 하락(음봉)일 때만 — 종가<SMA_200 → 청산(`SMA_200_BREAK`, strong), 종가<SMA_100 → 비중축소(`SMA_100_BREAK`, medium). `indicators`에 `SMA_100` 추가.
...
- **2026-06-17: 당일 미완성 봉(trailing NaN) 가드 확대 (Critical/High)** — NaN은 어떤 비교든 False라 신호를 조용히 무음화한다(`close > pivot`·`close < sma` → 항상 False). `last_valid_close` 공통 헬퍼(`src/tools/technical/df_guards.py`) 도입 후 소비처 전반에 가드: `criteria/engine.py`(진입가 NaN → 포지션 플랜 전체 오염), `criteria/vcp.py`(돌파 미탐지), `criteria/exit_rules.py`(손절 신호 무음화), `technical/components/chart_patterns.py` 디스패처(8개 패턴 함수 일괄). 회귀 테스트 추가.
- **2026-06-17: debate 수급 증거 양방향화 (High)** — `build_evidence_ledger`의 flow(수급) 증거가 매수 우세만 bull로 넣고 외인·기관 동시 매도는 누락하던 문제. 동시 매도를 bear(분산)로 추가해 약세 수급 신호가 bull/bear 토론에서 빠지지 않게 함(혼조·N/A는 그대로 증거 없음). LLM 판사에게 균형 잡힌 입력 제공.
