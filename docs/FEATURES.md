...
| 유형 | 예시 | 파싱 상태 | 영향도 계산 |
|------|------|----------|-----------|
| 실적발표 | 분기/연간 실적 | ✅ 구현됨 | 성장률, 마진 변화 |
| 유상증자 | 신주 발행 | ❌ 파싱 미구현 | 지분 희석률 |
| 전환사채 | CB 발행 | ❌ 파싱 미구현 | 잠재 희석률 |
| 공급계약 | 대규모 수주 | ❌ 파싱 미구현 | 매출 기여도 |

**구현 상태:**
- ✅ **영향도 계산 로직**: 모든 공시 유형 완성 (`ImpactCalculator`)
- ✅ **데이터 모델**: `DisclosureDetail` 클래스 완성
- ❌ **실제 파싱**: 주요사항보고서에서 공시 추출 **미구현**

**TODO:**
1. DART 주요사항보고서 파싱 (`/list.json` API)
2. 전환사채/유상증자/공급계약 정보 추출
3. 여러 건 공시 통합 로직 (총 희석률 계산)
