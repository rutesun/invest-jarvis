...
### Task 15: 턴어라운드 발굴 모드 (Screener Turnaround Mode) `P1`

**왜:**
check(추세 확인형)는 정확·안전하지만, ①늦게 발굴한 종목이 지금 들어가도 되는지, ②바닥에서 턴한 종목을 발굴하는지는 약하다. 사용자 실제 목표는 "추세 확인 후 이격 벌어지기 전 진입". 바닥 신호(bottom_watch)는 정밀도 벽으로 실패했으나, **"소량 테스트"로 프레이밍하면 정밀도 대신 재현율만 필요** → 실패 벽이 사라짐.

**깔때기 구조 (핵심):**
```
턴어라운드 스코어 스크리너(넓은 그물, 순위) → 상위 후보 소량 테스트 → check 확인 시 증량 / 이탈 시 손절
```
증량·손절 판단은 기존 check가 담당. 새로 만드는 건 그물(스코어)뿐.

**턴어라운드 스코어 (4마커, AND 아님·점수화·순위):**
- 급락 후 과매도 반등 (cRSI/RSI 과매도 훅)
- 20/50일선 재탈·지지
- 거래량 수반 양봉 (20일 평균 대비 급증 — 유일한 오실레이터 밖 독립축)
- 저점 높이기(higher low)

**검증 관문 결과 (2026-08-25):**
턴어라운드 스코어 자체는 나이브 기준선을 못 이김(예측 알파 없음). 그래서 **예측 신호가 아니라 발굴·해석 보조 도구**로 방향 확정 — 마커 내역+check확인+손절선을 제공하고 최종 판단(기사·시장)은 사용자가 한다.

**구현 완료 (개별 종목):**
- ✅ `src/tools/technical/turnaround.py` 코어 + check/screen(`--turnaround`)/brief 배선 (change record: `turnaround-signal.md`)
- ✅ KIS 외국인·기관 순매수 순위 버그 수정 (change record: `kis-investor-ranking-fix.md`)

**남은 과제 — 광역 바닥 발굴 (별도, 무거움):**
screen 발굴이 유니버스 미스매치로 변별력 낮음(네이버 급등주·순매수 리더 = 이미 오른 종목). '저점 근접+매집 전환' 프로토타입 결과, 순매수 리더 30 중 26이 이미 +15% 초과 상승, 저점권 4개는 블루칩뿐 — 성장주 V턴 없음. 진짜 바닥 발굴은 **"전종목/낙폭상위 → 저점 근접 1차 필터 → 외국인·기관 매집 '시작' 탐지"** 순서여야 하며, 전종목 마스터·무거운 스캔 설계가 필요. 새 세션 권장.

**상태:** 🟢 개별 종목 신호 구현 완료 · 🔵 광역 발굴은 별도 과제
**설계:** `docs/superpowers/specs/2026-08-24-bottom-watch-design.md`
**관련:** `src/tools/screener/`, `docs/worklog/bottom-watch-signal.md`, 메모리 "3차 발굴 엔진"
