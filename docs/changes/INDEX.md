...
|------|------|--------|------|----|--------|
| 2026-09-11 | feat | Draft | 바닥 다지기 그라데이션 — SMA50 아래 저점 높이기·강세 다이버전스·거래량 마름·모멘텀 개선 3개 동시 충족 시 상한(-15) 있는 가점 + `accumulate` 액션 추가. avoid(-90)→hold(+105) 급점프 전에 watch→accumulate 조기 관찰 밴드 생성. 추세 확인·신규진입 게이트 불변, 하락주 가짜 바닥 0(평가세트 튜닝) | - | [bottoming-gradient.md](bottoming-gradient.md) |
