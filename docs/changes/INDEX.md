...
|------|------|--------|------|----|--------|
| 2026-08-25 | fix | Draft | 외국인·기관 순매수 순위 복구 — `get_investor_ranking`이 화면코드 16174(오류)+정렬 파라미터 누락으로 항상 0건 조용히 반환하던 버그 수정(16449+FID_RANK_SORT_CLS_CODE, rt_cd 가드, 투자자별 정렬) | #{PR 번호} | [kis-investor-ranking-fix.md](kis-investor-ranking-fix.md) |
