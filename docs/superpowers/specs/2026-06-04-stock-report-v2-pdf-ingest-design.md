...
- **[CP2 결과] 청킹 잡음 필터(969 backfill 전 권장)**: `pdf_chunking` 핵심(표 원자성·부모 복원·중간 안 잘림·MAX_CHARS=1500 적정)은 실데이터 3개로 검증 통과. 라인 단위 차트 축 노이즈는 이미 필터됨. 잔여: ① 차트가 markdown 파이프 블록으로 흘러 **가짜 "표" 청크**(`|0<br>200,000<br>|` 등) 생성(특히 2단 차트多 하나노트), ② 표 사이에 낀 **출처줄**(`자료: …`)이 tiny 청크로 잔존. 후속 필터: ① 셀이 거의 숫자/축토큰이고 헤더 단어 없는 파이프블록은 표가 아니라 노이즈로 스킵, ② 출처줄은 앞 청크에 병합. MIN_CHARS는 ①②이후 ~120으로 낮추는 것 검토.
