...
def run_evaluation(
        ...
        for name, value in metrics.items():
            if isinstance(value, (int, float)):
                ...
                for detail in value:
                    ...
                    print(
                        f"     {match_icon} {detail.get('expected')} → {detail.get('reason', 'N/A')}"
                    )

        # 결과 저장 (JSON 직렬화 가능하도록)
        serializable_metrics = {
            k: v for k, v in metrics.items() if isinstance(v, (int, float, str, bool, list))
        }
        case_result = {
            "id": case_id,
            "name": case_name,
    ...
    # 전체 요약
    results["summary"] = {
        name: sum(scores) / len(scores) if scores else 0.0 for name, scores in all_scores.items()
    }
