...
5. **클러스터링 일관성**: `themes`는 이슈 그룹화의 키(Key)가 됩니다.
...
6. **카테고리 선택 (category)**:
   아래 20개 중 이슈를 가장 잘 대표하는 하나를 선택하세요.
   반도체 | 디스플레이 | 이차전지 | 소재/화학 | 자동차 | 조선/중공업 | 방산 |
   AI/소프트웨어 | 통신 | 바이오/제약 | 유통/소비재 | K-푸드 | 엔터/미디어 |
   운송/물류 | 에너지 | 건설/부동산 | 금융/보험 | 매크로 | 정책/규제 | 기타
...
2. **공격적 클러스터링 (Aggressive Clustering)**: 유사한 메시지는 반드시 하나의 이슈로 통합하세요.
   - 목표: **평균 1.5개 이상의 메시지를 하나의 이슈로 통합** (avg_sources ≥ 1.5)
   - 단순 기업별 나열이 아니라 '현상' 위주로 묶으세요
   - 예: '오라클-블룸 협력'과 '데이터센터 전력 인프라 부족'은 하나의 이슈로 통합
   - 예: 'HBM 공급 부족', 'HBM 가격 인상', 'HBM 검증 통과'는 모두 'HBM 업사이클'로 통합
...
2. **공격적 클러스터링 (Aggressive Clustering)**: 유사한 메시지는 반드시 하나의 이슈로 통합하세요.
   ...
   **목표 지표**:
   - avg_sources ≥ 2.0 (평균 2개 이상의 메시지를 하나의 이슈로 통합)
   - 최종 이슈 개수는 입력 메시지의 30-40% 이내

   **반드시 통합해야 할 경우**:
   - 같은 기업의 관련 뉴스 (예: 실적 + 목표가 변경 + 수주 소식)
   - 같은 산업 트렌드 (예: HBM 공급부족 + HBM 가격인상 + HBM 검증통과 → "HBM 업사이클")
   - 인과관계가 있는 뉴스 (예: 오라클-블룸 협력 + 데이터센터 전력 부족 → "AI 전력 인프라 투자 가속")
   - 동일 테마의 복수 종목 (예: 삼성HBM + SK하이닉스HBM + 마이크론HBM → "HBM 업사이클")

   **통합하지 말아야 할 경우**:
   - 명확히 다른 섹터 (예: 반도체 vs 자동차)
   - 반대 방향성 (예: 호재 vs 악재)
   - 전혀 다른 투자 내러티브 (예: 전력 인프라 vs 공급망 리쇼어링)

3. **테마 재사용 (Theme Reuse)**: 같은 투자 내러티브는 같은 테마명을 재사용하세요.
   - 목표: 고유 테마 수 ≤ 이슈 수 × 1.5
   - 예: 3개 이슈가 모두 "AI 전력 병목"을 다룬다면, 같은 테마명 사용
   - 섹터 명칭(반도체, 음식료) 대신 '투자 내러티브'로 작명
     - 추천: ["AI 전력 병목 해소"], ["K-푸드 글로벌 피크아웃 우려 불식"], ["DRAM 선단공정 전환 가속화"]
...
MAP_SYSTEM_PROMPT = MAP_SYSTEM_PROMPT_V6
...
# System/User 분리 버전 (프롬프트 캐싱용)
SHUFFLE_SYSTEM_PROMPT_V1 = """다음은 여러 청크에서 추출된 테마들입니다.
유사한 의미의 테마들을 하나의 정규화된 이름으로 통합하세요.
...
SHUFFLE_SYSTEM_PROMPT_V2 = """당신은 한국 금융 시장 전문 애널리스트입니다.
동일한 카테고리 내에서 추출된 테마들을 정규화(통합)합니다.

**핵심 목표: 공격적 통합**
같은 산업 흐름, 같은 밸류체인, 같은 트렌드를 다루는 테마는 반드시 통합하세요.
테마명이 달라도 이슈 내용이 같은 흐름이면 합칩니다.

**통합 기준** (하나라도 해당하면 통합):
1. 같은 밸류체인: "HBM 가격", "AI 메모리 수요", "메모리 반도체 업사이클" → 같은 흐름
2. 같은 이벤트: "삼성 실적", "삼성 반도체 매출" → 같은 이벤트
3. 같은 트렌드: "AI 전력 수요", "데이터센터 전력" → 같은 트렌드
4. 인과관계: "금리 인하" + "성장주 반등" → 하나의 흐름
...
- "삼성전자 HBM3E 양산"
- "SK하이닉스 엔비디아 공급"
출력: {{"groups": [{{"normalized": "AI 메모리/HBM 업사이클", "originals": ["HBM 가격 상승", "AI 메모리 수요 급증", "삼성전자 HBM3E 양산", "SK하이닉스 엔비디아 공급"]}}]}}

입력:
- "2차전지 소재 가격"
- "리튬 가격 반등"
- "양극재 수출 증가"
출력: {{"groups": [{{"normalized": "2차전지 소재 가격 반등", "originals": ["2차전지 소재 가격", "리튬 가격 반등", "양극재 수출 증가"]}}]}}

**예시 (나쁜 통합 - 이렇게 하지 마세요)**:
- "AI 반도체" + "바이오 신약" → ❌ 완전히 다른 산업
- "반도체" 하나로 전부 통합 → ❌ 너무 광범위

**출력**: 제공된 함수 스키마(Tool Calling) 형식에 맞추어 반환하세요."""

SHUFFLE_SYSTEM_PROMPT = SHUFFLE_SYSTEM_PROMPT_V2

SHUFFLE_USER_PROMPT_V1 = """**입력 테마 리스트**:
{themes}

위 테마들을 정규화하세요."""

SHUFFLE_USER_PROMPT_V2 = """**카테고리**: {category}

**테마별 이슈 요약**:
{themes_with_context}

같은 흐름의 테마를 공격적으로 통합하세요. 테마명이 달라도 이슈 내용이 같은 산업 흐름이면 합칩니다."""

SHUFFLE_USER_PROMPT = SHUFFLE_USER_PROMPT_V2
...
1. **테마 간 관계 파악**
   ...
   - 연결 관계: 서로 연결된 테마들을 파악하세요
     예: "AI 인프라 투자 확대 → HBM 수요 증가"

   - 대비 관계: 상반되는 테마들을 파악하세요
     예: "AI 섹터 강세 vs 이차전지 약세"

   - 선후 관계: 선후 관계가 있는 테마들을 파악하세요
     예: "엔비디아 독점 완화 → GPU 공급망 재편"

2. **매크로 데이터와 연결**

   - VIX, Fear & Greed 지수가 테마들을 뒷받침/반박하는지 분석하세요
   - 미국/한국 시장 차이와 관련된 테마를 파악하세요
   - 환율이 특정 섹터에 미치는 영향을 분석하세요

3. **우선순위 결정**

   - 오늘의 핵심 테마를 선정하세요 (이슈 수, 영향력, 시의성 고려)
   - 가장 주목해야 할 섹터를 선정하세요
   - 수혜/리스크를 받는 종목/섹터를 파악하세요
