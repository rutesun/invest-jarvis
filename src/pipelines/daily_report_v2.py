...
class DailyReportV2Pipeline:
    ...
    async def run(
        ...
        logger.info("=== Daily Report V2: %d개 Stage 실행 (%s) ===",
                    len(stages_to_run), " → ".join(stages_to_run))
    ...
    async def _run_stages(
        ...
        for stage_name in stages:
            if stage_name == "ingest":
                ...
                cache_path = cache.save(STAGE_CACHE_KEYS["ingest"], ingest_result.model_dump())
                ...
                cache_path = cache.save(STAGE_CACHE_KEYS["map"], [i.model_dump() for i in map_result])
                ...
                cache_path = cache.save(STAGE_CACHE_KEYS["shuffle"], shuffle_result.model_dump())
                ...
                cache_path = cache.save(STAGE_CACHE_KEYS["catalyst"], [c.model_dump() for c in catalyst_result])
                ...
                cache_path = cache.save(STAGE_CACHE_KEYS["synthesize"], report.model_dump())
