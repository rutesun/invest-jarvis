...
@pytest.mark.asyncio
async def test_screener_pipeline_theme_aggregation(mock_universe_builder, mock_evidence_collector, mock_news_tool):
    ...
    mock_evidence_collector.collect_and_score.return_value = [
        ScreenerEvidence(
            stock=UniverseStock(ticker="005930", name="삼성전자", market="KOSPI", sources=["theme"], theme="AI/반도체", theme_change_rate=3.2),
            accumulation_score=12.0, momentum_total=47.0, total_score=21.0, vol_ratio=3.5, rank=1,
        ),
        ScreenerEvidence(
            stock=UniverseStock(ticker="000660", name="SK하이닉스", market="KOSPI", sources=["theme"], theme="AI/반도체", theme_change_rate=3.2),
            accumulation_score=8.0, momentum_total=35.0, total_score=15.0, vol_ratio=2.5, rank=2,
        ),
        ScreenerEvidence(
            stock=UniverseStock(ticker="370330", name="현대차", market="KOSPI", sources=["theme"], theme="전기차", theme_change_rate=1.5),
            accumulation_score=5.0, momentum_total=25.0, total_score=10.0, vol_ratio=1.8, rank=3,
        ),
    ]
...
@pytest.mark.asyncio
async def test_screener_pipeline_format_output(mock_universe_builder, mock_evidence_collector, mock_news_tool):
    """Test format_output produces valid markdown."""
    mock_evidence_collector.collect_and_score.return_value = [
        ScreenerEvidence(
            stock=UniverseStock(ticker="005930", name="삼성전자", market="KOSPI", sources=["theme", "rise_rank"]),
            accumulation_score=12.0, momentum_total=47.0, total_score=21.0, vol_ratio=3.5, rank=1,
        ),
    ]
...
@pytest.mark.asyncio
async def test_screener_pipeline_save_report(mock_universe_builder, mock_evidence_collector, mock_news_tool, tmp_path):
