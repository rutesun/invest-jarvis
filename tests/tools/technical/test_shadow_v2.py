...
def test_stage2_up_deterioration_demotes_to_watch():
    ...
    action, _ = _decide(regime="trend", st_up=True, setup_score=-10, sma20_break_2d=True)
    assert action == "watch"
...
def test_fresh_flip_alone_does_not_buy():
    ...
    action, entry = _decide(regime="trend", st_up=True, fresh_buy_flip=True, setup_score=50)
    assert action == "hold"
    assert entry is False
